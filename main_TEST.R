...
for (organism_data in organisms_data) {
  ...
  for (substitution_matrix in substitution_matrices) {
    cl <- makeCluster(cores-1, outfile="debug_file.txt")
    ...
    system.time({
      ...
      result = pblapply(cl=cl,
                        ...
                        FUN=function(idx) {
                          ...
                          min_self_align_score <- apply(result_all[7], 2, min)
                          
                          if (min_self_align_score == 0) {
                            min_self_align_score <- .000001
                          }

                          #print("min_self_align_score")
                          #print(min_self_align_score)

                          # Uniqueness score calculation
                          for (peptide_idx in 1:nrow(result_all)) {
                            result_all[peptide_idx, 7] <- (1 - (result_all[peptide_idx, 7] / min_self_align_score))
                          }
                          
                          best_score_row_idx <- apply(result_all[7], 2, which.min)

                          # get only 1 shortest distance for each protein
                          result = as.data.frame(matrix(nrow=1, ncol=9))
                          result[1, 1] <- result_all[best_score_row_idx, 1]
                          result[1, 2] <- result_all[best_score_row_idx, 2]
                          result[1, 3] <- result_all[best_score_row_idx, 3]
                          result[1, 4] <- result_all[best_score_row_idx, 4]
                          result[1, 5] <- result_all[best_score_row_idx, 5]
                          result[1, 6] <- result_all[best_score_row_idx, 6]
                          result[1, 7] <- result_all[best_score_row_idx, 7]
                          result[1, 8] <- result_all[best_score_row_idx, 8]
                          result[1, 9] <- result_all[best_score_row_idx, 9]
  ...
  for (idx in 1:max(rank)) {
    ...
    df_pareto_front_points <- df_objectives[pareto_front_points_indicies, 
                                            ...
                                            c('Probability', 'pair_align_score')]
